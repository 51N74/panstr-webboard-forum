...
import db, { liveNotifications, liveUnreadCount } from "../../lib/storage/indexedDB";
...
const Notifications = ({ compact = false }) => {
          ...
          for (const reply of replies) {
            if (reply.pubkey !== user.pubkey) {
              ...
              const alreadyNotified = existing.some(n =>
                n.type === 'reply' && n.eventId === reply.id
              );
      ...
      if (notificationSettings.mentions) {
        ...
        for (const mention of mentions) {
          if (mention.pubkey !== user.pubkey) {
            const existing = await db.getNotifications(user.pubkey, 100, false);
            const alreadyNotified = existing.some(n =>
              n.type === 'mention' && n.eventId === mention.id
            );
      ...
      if (notificationSettings.zaps) {
        ...
        for (const zap of zapReceipts) {
          ...
          if (!alreadyNotified) {
            ...
            const description = zap.tags.find(t => t[0] === "description")?.[1] || "";
            let zapRequest = null;
  ...
  const getNotificationColor = (type) => {
    switch (type) {
      case 'reply': return 'text-blue-600 bg-blue-100';
      case 'mention': return 'text-purple-600 bg-purple-100';
      case 'zap': return 'text-yellow-600 bg-yellow-100';
