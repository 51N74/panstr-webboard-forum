...
export default function GeneralForum() {
  ...
  const handleSendReply = useCallback(async () => {
    ...
    try {
      ...
      // Create reply event (unsigned object)
      const replyEvent = createReplyEvent(
        replyToEvent,
        replyContent,
        null, // signing will be handled by publishToPool (browser extension or stored key)
        [
          ["t", "general"],
          ["t", "forum"],
        ],
      ...
      // Basic client-side validation: ensure required tags exist
      const hasETag =
        Array.isArray(replyEvent.tags) &&
        replyEvent.tags.some((t) => t[0] === "e");
      const hasPTag =
        Array.isArray(replyEvent.tags) &&
        replyEvent.tags.some((t) => t[0] === "p");
      if (!hasETag || !hasPTag) {
        throw new Error("Reply is missing required e/p tags");
      }

      // Publish the reply using publishToPool.
      // publishToPool signature: publishToPool(pool, relayUrls, privKey, content, { kind, tags })
      const publishedEvent = await publishToPool(
        pool,
        undefined,
        null, // rely on browser extension signer if available or stored key in publishToPool
        replyEvent.content,
        { kind: replyEvent.kind || 1, tags: replyEvent.tags },
      );
  ...
  const getConnectedCount = () => {
    return Object.values(relayStatuses).filter((status) => status.connected)
      .length;
  };
        ...
        <div className="bg-white rounded-lg shadow-sm p-6">
          ...
          <div className="flex items-center justify-between mb-6">
            <div className="flex items-center space-x-4">
              ...
              {events.length > 0 && (
                <div className="text-sm text-gray-600">
                  Posts: <span className="font-bold">{events.length}</span>
                </div>
              )}
            </div>
            <button
